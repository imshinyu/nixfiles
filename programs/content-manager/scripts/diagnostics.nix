...
''
  ...
  run_diagnostics() {
    ...
    echo "GL renderer: $(${pkgs.mesa-demos}/bin/glxinfo | grep "OpenGL renderer" || echo "glxinfo not found")"
    ...
    if ${pkgs.vulkan-tools}/bin/vulkaninfo --summary &>/dev/null; then
      ...
      if ${pkgs.mesa-demos}/bin/glxinfo | grep -i "vendor" | grep -i "nvidia" &>/dev/null; then
        echo "NVIDIA GPU detected. Make sure to install nvidia-drivers with Vulkan support."
      elif ${pkgs.mesa-demos}/bin/glxinfo | grep -i "vendor" | grep -i "amd\|ati" &>/dev/null; then
        echo "AMD GPU detected. Make sure to install amdvlk or RADV Vulkan drivers."
      elif ${pkgs.mesa-demos}/bin/glxinfo | grep -i "vendor" | grep -i "intel" &>/dev/null; then
  ...
  EOF
    ...
    if [ -f "$AC_COMPATDATA/pfx/drive_c/users/steamuser/AppData/Local/AcTools Content Manager/settings.ini" ]; then
      ...
      if ! grep -q "videMemorySize=" "$cm_settings"; then
        ...
        vram=$(${pkgs.mesa-demos}/bin/glxinfo | grep "Video memory" | head -n1 | grep -o '[0-9]\+' | head -n1)
        if [ -n "$vram" ]; then
